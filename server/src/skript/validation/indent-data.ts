...
import { SkriptContext } from './skript-context';
...
export class IndentData {
	...
	*/
	...
	/**returns hasColon only for now! */
	static getIndentData(line: string): IndentData {
		const result = new IndentData();
		result.hasColon = line.endsWith(':');
		result.endIndex = this.getIndentationEndIndex(line);
		return result;
	}
